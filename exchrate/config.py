...
class CurrencyCode:
    ...
    def __init__(self, filename=None):
        ...
        try:
            s = files("exchrate").joinpath('data').joinpath('iso_4217.xml').read_text()
        except OSError:
        ...
        if s:
            ...
            self._ccy_codes = {
                ...
                for ccy_entry in xml.fromstring(s).iter('CcyNtry')
                if len(ccy_entry) == 5
            }
    ...
    def get_ccy_num_codes(self, ccy_char_codes):
        ...
        return {
            ccy_char_code: self._ccy_codes[ccy_char_code]['ccy_code']
            for ccy_char_code in ccy_char_codes
            if ccy_char_code in self._ccy_codes.keys()
        }

    def get_ccy_codes(self):
        '''returns literal: digit code pairs'''
        return {k: v['ccy_code'] for k, v in self._ccy_codes.items()}
